...
//        testSendPushWithCallback();
     ...
     * @param content
     */
    public static void sendAppMsg(String token, String content){
        String APP_KEY_JIGUANG = Global.getConfig("APP_KEY_JIGUANG");
        String MASTER_SECRET_JIGUANG = Global.getConfig("MASTER_SECRET_JIGUANG");
        JPushClient jpushClient = new JPushClient(MASTER_SECRET_JIGUANG, APP_KEY_JIGUANG, null, ClientConfig.getInstance());
        PushPayload payload = SmsController.buildPushObject_all_alias_alert(token,content);
        try {
            PushResult result = jpushClient.sendPush(payload);
            System.out.println("成功推送+token=="+token);
        } catch (cn.jiguang.common.resp.APIConnectionException e) {
            System.out.println("Connection error, should retry later" + e);

        } catch (APIRequestException e) {
            System.out.println("CShould review the error, and fix the request" + e +"HTTP Status:"+e.getStatus()+"Error Code: " + e.getErrorCode()+"Error Message: " + e.getErrorMessage());
        }
    }
...
//        }
    ...
    public static PushPayload buildPushObject_all_alias_alert() {
        return PushPayload.newBuilder()
                ...
                .setAudience(Audience.alias("35dbb30d3b8145388938060fb3f8c9b2"))
