...
<div id="tt" class="easyui-tabs">   
    ...
    <div title="执行参数任务" data-options="refreshable: false">   
   		<div id="tbp" style="padding:5px;height:auto">
			<a href="javascript:void(0)" class="easyui-linkbutton" iconCls="icon-standard-clock-edit" plain="true" onclick="updp();">修改参数</a>
			<a href="javascript:void(0)" class="easyui-linkbutton" iconCls="icon-standard-clock-play" plain="true" onclick="startNowWithP();">立即运行一次</a>
		</div>
        <table id="dg_runningwithparam"></table>    
    </div>   
</div>  
...
$('#tt').tabs({    
    ...
    onSelect:function(title,index){    
    	if(index==1){
    		...
    		dg_runningwithparam=$('#dg_runningwithparam').datagrid({    
    		    url:'${ctx}/system/scheduleJob/json', 
    		    fit : true,
    			fitColumns : true,
    			border : false,
    		    ...
    		    columns:[[    
    		        {field:'name',title:'任务名',sortable:true,width:30},    
    		        {field:'group',title:'任务组',sortable:true,width:30},
    		        {field:'param',title:'参数',sortable:true,width:70,editor: "text"},
    		        {field:'status',title:'状态',sortable:true,width:30},
    		        {field:'className',title:'任务类',sortable:true},
    		        {field:'description',title:'描述',sortable:true}
    		    ]],
...
function updp(){
	var row = dg_runningwithparam.datagrid('getSelected');
	if(rowIsNull(row)) return;
	var rowIndex=dg_runningwithparam.datagrid('getRowIndex',row);
	dg_runningwithparam.datagrid('beginEdit',rowIndex);
}
...
function startNow(){
	...
	parent.$.messager.confirm('提示', '确定要立即运行一次该任务？', function(data){
		if (data){
			$.ajax({
				type:'get',
				url:"${ctx}/system/scheduleJob/"+encodeURI(encodeURI(row.name))+"/"+row.group+"/startNow",
				success: function(data){
					if(data=='success'){
						//dg.datagrid('reload');
						parent.$.messager.show({ title : "提示",msg: "操作成功！", position: "bottomRight" });
...
function startNowWithP(){
	...
	parent.$.messager.confirm('提示', '确定要立即运行一次该任务？', function(data){
		if (data){
			$.ajax({
				type:'post',
				url:"${ctx}/system/scheduleJob/"+encodeURI(encodeURI(row.name))+"/"+row.group+"/stratNowWithParam",
