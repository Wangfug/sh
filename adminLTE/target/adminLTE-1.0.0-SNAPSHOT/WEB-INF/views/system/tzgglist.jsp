...
<div id="tb" style="padding:5px;height:auto">
        <div>
        	<form id="searchFrom" action="">
       	        <input type="text" name="filter_tzbt" class="easyui-validatebox" data-options="width:150,prompt: '标题'"/>
...
$(function(){   
    ...
    columns:[[    
		...
		{field:'tzbt',title:'标题',sortable:true,width:100,formatter : function(value, row, index) {
			return '<a href="javascript:;" onclick="viewtz('+row.id+',\''+row.tzbt+'\');">'+value+'</a>';
		}},
        {field:'zdbz',title:'置顶',sortable:true,width:100,formatter : function(value, row, index) {
        	if(value=="0"){
        		return '否';
        	}else{
        		return '是';
        ...
        {field:'scbz',title:'注销标记',sortable:true,width:100,formatter : function(value, row, index) {
        	if(value=="0"){
        		return '否';
        	}else{
        		return '是';
...
function del(){
	...
	parent.$.messager.confirm('提示', '您确定要删除这条记录吗？', function(data){
		if (data){
			$.ajax({
				type:'get',
				url:"${ctx}/system/tzgg/delete/"+row.id,
...
function upd(){
	...
	d=$("#dlg").dialog({   
	    ...
	    width: 680,    
	    height: 550,    
	    href:'${ctx}/system/tzgg/update/'+row.id,
	    maximizable:true,
	    modal:true,
	    buttons:[{
			text:'修改',
